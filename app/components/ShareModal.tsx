...
export const ShareModal: React.FC<ShareModalProps> = ({ isOpen, onClose, song }) => {
  ...
  const defaultShareText = `🎵 "${song.title}" ${song.style ? `(${song.style})` : ''} - Made with ${STUDIO.name}`;
  const twitterText = `🔥 Just created "${song.title}" with ${STUDIO.name}! ${song.style ? `#${song.style.replace(/\s+/g, '')}` : ''} #AIMusic`;
  const redditTitle = `[AI Music] ${song.title} - ${song.style || 'Original'} | Created with ${STUDIO.name}`;
  const whatsAppText = `🎧 Listen to this AI-generated song!\n\n"${song.title}" by ${song.creator || 'Unknown Artist'}\n${song.style ? `Genre: ${song.style}` : ''}\n\nMade with ${STUDIO.name}.`;
  const telegramText = `🎵 "${song.title}" by ${song.creator || 'Unknown Artist'}\n${song.style ? `🎸 ${song.style}` : ''}\n\n🤖 Made with ${STUDIO.name}`;
  const linkedInText = `Check out this AI-generated music: "${song.title}" - Created with ${STUDIO.name}. #AIMusic #MusicTech #OpenSource`;
